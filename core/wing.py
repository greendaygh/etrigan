...
## point map 가져오는 함수 
    ...
    @torch.no_grad()
    def get_pointmap(self, x):
        landmarks = self.get_landmark(x)
        b,_,h,w = x.shape
        ch = landmarks.shape[1]
        result = torch.zeros((b,ch,h,w),dtype=x.dtype, device=x.device) 
        for i in range(b):
            cords = landmarks[i]
            for j, point in enumerate(cords):
                xx, yy = torch.meshgrid(torch.arange(w), torch.arange(h))
                result[i,j,:,:] = torch.exp(-((yy - point[0]) ** 2 + (xx - point[1]) ** 2) / (2 * 6 ** 2))
        return result
