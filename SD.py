...
# AutoencoderKL --> 처음 img --> E에서 E (그림)
# UNet, Schedular 3개 필요 
...
class StableDiffusion(nn.Module):
    def __init__(self, device, sd_version='2.0', hf_key=None):
        ...
        ## 지금은 여러개 버전이 있지만 1.5 사용 
        ## 2.0 은 이미지 사이즈 776 이고 택스트 인코딩 향상
    ...
    # latents 는 Z' 즉, 여전히 노이즈
    # 이걸 디코딩 해야 이미지 볼 수 있음
    @torch.no_grad()
    def decode_latents(self, latents):
        latents = 1 / 0.18215 * latents
        imgs = self.vae.decode(latents).sample
        imgs = (imgs / 2 + 0.5).clamp(0, 1) # 사이즈 맞춤
        return imgs 
    ...
    def text2image(self, input_img, prompts, negative_prompts='', height=512, width=512, num_inference_steps=50,
        ...
        # 디폴트가 512, encoding 해서 만든 Z 사이즈가 64라서 
        ...
        # 스케줄러 셋업
        # ddim 은 디폴트로 50회 원래는 1000인데 20씩 건너뛰어서 980, 960, ...
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                # 프롬프트 2개 입력 예정이라 (입력, 네거티브)
                latent_model_input = torch.cat([latent] * 2)
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                # predict the noise residual
                # unet 아웃풋은 딕션너리로 나오는데 sample은 노이즈가 출력
                # 아래 prev_sample은 이전 스텝의 X_t-1
