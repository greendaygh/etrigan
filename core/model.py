...
class Generator(nn.Module):
    ...
    def forward(self, x, s, masks=None):
        x = self.from_rgb(x)
        cache = {}
        for block in self.encode: # sel.encode: 인코더 블럭들의 모임 (Unet 구조에서), 
            if (masks is not None) and (x.size(2) in [32, 64, 128]): # 32, 64, 128 일 때, 얼굴이 뭉개지기 전 상태의 feature map 정보
                cache[x.size(2)] = x # encoder를 통과한 feature map이 cache에 저장중 
            x = block(x)
...
def build_model(args):
    ...
    if args.w_hpf > 0:
        ...
        fan.get_pointmap = fan.module.get_pointmap # parallel 로 바꾸면 모듈로 들어가서???
